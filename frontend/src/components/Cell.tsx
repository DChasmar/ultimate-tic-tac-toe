...
const Cell: React.FC<CellProps> = ({
  ...
  const handleAddSymbolToCell =
    (
      nextSubGridNumberCode: number,
      subGridRow: number,
    ...
    () => {
      ...
      const newSubGridsCompleted = updateSubGridsCompleted(
        newGridValues[subGridRow][subGridColumn],
        subGridsCompleted,
        newGridCode,
        whoseTurn
      );
      ...
      const isGameOver =
        checkWinningCombinations(newSubGridsCompleted[whoseTurn]) ||
        newSubGridsCompleted["full"].size +
          newSubGridsCompleted["X"].size +
          newSubGridsCompleted["O"].size ===
          9;
      if (isGameOver) {
        setNextSubGrid(new Set<number>());
        setGameOver(true);
        return;
      }

      const newNextSubGrid = updateNextSubGrid(
        nextSubGridNumberCode,
        newSubGridsCompleted
      );
